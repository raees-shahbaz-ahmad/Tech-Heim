import React from 'react';
import { useNavigate } from 'react-router';
import "../Blogs/Blogs.css";

const Blogs = () => {

    const navigate = useNavigate();
    const handleViewAllBtn = () => {
        navigate(`/blog`);
    }

    return (
        <div className="new-products">
            <div className="new-products-heading-content">
                <div className="new-products-heading">Our Blogs</div>
                <div onClick={handleViewAllBtn} className="new-products-viewall">View all
                    <svg className="new-products-angle" viewBox="0 0 24 24" fill="currentcolor"
                        xmlns="http://www.w3.org/2000/svg">
                        <path
                            d="M10.7397 16.2799C10.5497 16.2799 10.3597 16.2099 10.2097 16.0599C9.91969 15.7699 9.91969 15.2899 10.2097 14.9999L13.2097 11.9999L10.2097 8.99991C9.91969 8.70991 9.91969 8.22991 10.2097 7.93991C10.4997 7.64991 10.9797 7.64991 11.2697 7.93991L14.7997 11.4699C15.0897 11.7599 15.0897 12.2399 14.7997 12.5299L11.2697 16.0599C11.1197 16.2099 10.9297 16.2799 10.7397 16.2799Z"
                            fill="#0C0C0C" />
                    </svg>
                </div>
            </div>

            <div className="sepration" style={{ border: "1px solid #b4b4b4", marginTop: "15px" }}></div>

            <div className="blog-container">
                <div onClick={handleViewAllBtn} className="left-blog">
                    <img className="left-blog-img" src="./left-blog-img.png" />
                    <div className="left-blog-internal-content">
                        <div className="left-blog-date-time">
                            <div className="date">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="currentcolor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M6.66602 4.79167C6.32435 4.79167 6.04102 4.50833 6.04102 4.16667V1.66667C6.04102 1.325 6.32435 1.04167 6.66602 1.04167C7.00768 1.04167 7.29102 1.325 7.29102 1.66667V4.16667C7.29102 4.50833 7.00768 4.79167 6.66602 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.334 4.79167C12.9923 4.79167 12.709 4.50833 12.709 4.16667V1.66667C12.709 1.325 12.9923 1.04167 13.334 1.04167C13.6757 1.04167 13.959 1.325 13.959 1.66667V4.16667C13.959 4.50833 13.6757 4.79167 13.334 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 12.0833C6.975 12.0833 6.86667 12.0583 6.76667 12.0167C6.65833 11.975 6.575 11.9167 6.49167 11.8417C6.34167 11.6833 6.25 11.475 6.25 11.25C6.25 11.1417 6.275 11.0333 6.31667 10.9333C6.35833 10.8333 6.41667 10.7417 6.49167 10.6583C6.575 10.5833 6.65833 10.525 6.76667 10.4833C7.06667 10.3583 7.44167 10.425 7.675 10.6583C7.825 10.8167 7.91667 11.0333 7.91667 11.25C7.91667 11.3 7.90833 11.3583 7.9 11.4167C7.89167 11.4667 7.875 11.5167 7.85 11.5667C7.83333 11.6167 7.80833 11.6667 7.775 11.7167C7.75 11.7583 7.70833 11.8 7.675 11.8417C7.51667 11.9917 7.3 12.0833 7.08333 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 12.0833C9.89102 12.0833 9.78268 12.0583 9.68268 12.0167C9.57435 11.975 9.49102 11.9167 9.40768 11.8417C9.25768 11.6833 9.16602 11.475 9.16602 11.25C9.16602 11.1417 9.19102 11.0333 9.23268 10.9333C9.27435 10.8333 9.33268 10.7417 9.40768 10.6583C9.49102 10.5833 9.57435 10.525 9.68268 10.4833C9.98268 10.35 10.3577 10.425 10.591 10.6583C10.741 10.8167 10.8327 11.0333 10.8327 11.25C10.8327 11.3 10.8243 11.3583 10.816 11.4167C10.8077 11.4667 10.791 11.5167 10.766 11.5667C10.7493 11.6167 10.7243 11.6667 10.691 11.7167C10.666 11.7583 10.6243 11.8 10.591 11.8417C10.4327 11.9917 10.216 12.0833 9.99935 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 12.0833C12.809 12.0833 12.7007 12.0583 12.6007 12.0167C12.4923 11.975 12.409 11.9167 12.3257 11.8417C12.2923 11.8 12.259 11.7583 12.2256 11.7167C12.1923 11.6667 12.1673 11.6167 12.1507 11.5667C12.1257 11.5167 12.109 11.4667 12.1007 11.4167C12.0923 11.3583 12.084 11.3 12.084 11.25C12.084 11.0333 12.1757 10.8167 12.3257 10.6583C12.409 10.5833 12.4923 10.525 12.6007 10.4833C12.909 10.35 13.2757 10.425 13.509 10.6583C13.659 10.8167 13.7507 11.0333 13.7507 11.25C13.7507 11.3 13.7423 11.3583 13.734 11.4167C13.7257 11.4667 13.709 11.5167 13.684 11.5667C13.6673 11.6167 13.6423 11.6667 13.609 11.7167C13.584 11.7583 13.5423 11.8 13.509 11.8417C13.3507 11.9917 13.134 12.0833 12.9173 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 15C6.975 15 6.86667 14.975 6.76667 14.9333C6.66667 14.8917 6.575 14.8333 6.49167 14.7583C6.34167 14.6 6.25 14.3833 6.25 14.1667C6.25 14.0583 6.275 13.95 6.31667 13.85C6.35833 13.7417 6.41667 13.65 6.49167 13.575C6.8 13.2667 7.36667 13.2667 7.675 13.575C7.825 13.7333 7.91667 13.95 7.91667 14.1667C7.91667 14.3833 7.825 14.6 7.675 14.7583C7.51667 14.9083 7.3 15 7.08333 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 15C9.78268 15 9.56602 14.9083 9.40768 14.7583C9.25768 14.6 9.16602 14.3833 9.16602 14.1667C9.16602 14.0583 9.19102 13.95 9.23268 13.85C9.27435 13.7417 9.33268 13.65 9.40768 13.575C9.71602 13.2667 10.2827 13.2667 10.591 13.575C10.666 13.65 10.7243 13.7417 10.766 13.85C10.8077 13.95 10.8327 14.0583 10.8327 14.1667C10.8327 14.3833 10.741 14.6 10.591 14.7583C10.4327 14.9083 10.216 15 9.99935 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 15C12.7007 15 12.484 14.9084 12.3257 14.7584C12.2507 14.6834 12.1923 14.5917 12.1507 14.4834C12.109 14.3834 12.084 14.275 12.084 14.1667C12.084 14.0584 12.109 13.95 12.1507 13.85C12.1923 13.7417 12.2507 13.65 12.3257 13.575C12.5173 13.3834 12.809 13.2917 13.0757 13.35C13.134 13.3584 13.184 13.375 13.234 13.4C13.284 13.4167 13.334 13.4417 13.384 13.475C13.4257 13.5 13.4673 13.5417 13.509 13.575C13.659 13.7334 13.7507 13.95 13.7507 14.1667C13.7507 14.3834 13.659 14.6 13.509 14.7584C13.3507 14.9084 13.134 15 12.9173 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M17.0827 8.19997H2.91602C2.57435 8.19997 2.29102 7.91664 2.29102 7.57497C2.29102 7.2333 2.57435 6.94997 2.91602 6.94997H17.0827C17.4243 6.94997 17.7077 7.2333 17.7077 7.57497C17.7077 7.91664 17.4243 8.19997 17.0827 8.19997Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.3333 18.9583H6.66667C3.625 18.9583 1.875 17.2083 1.875 14.1667V7.08333C1.875 4.04167 3.625 2.29167 6.66667 2.29167H13.3333C16.375 2.29167 18.125 4.04167 18.125 7.08333V14.1667C18.125 17.2083 16.375 18.9583 13.3333 18.9583ZM6.66667 3.54167C4.28333 3.54167 3.125 4.7 3.125 7.08333V14.1667C3.125 16.55 4.28333 17.7083 6.66667 17.7083H13.3333C15.7167 17.7083 16.875 16.55 16.875 14.1667V7.08333C16.875 4.7 15.7167 3.54167 13.3333 3.54167H6.66667Z"
                                        fill="#717171" />
                                </svg>
                                August , 8 , 2023
                            </div>

                            <div className="time">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="currentcolor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M10.0007 18.9583C5.63398 18.9583 2.08398 15.4083 2.08398 11.0417C2.08398 6.675 5.63398 3.125 10.0007 3.125C14.3673 3.125 17.9173 6.675 17.9173 11.0417C17.9173 15.4083 14.3673 18.9583 10.0007 18.9583ZM10.0007 4.375C6.32565 4.375 3.33398 7.36667 3.33398 11.0417C3.33398 14.7167 6.32565 17.7083 10.0007 17.7083C13.6757 17.7083 16.6673 14.7167 16.6673 11.0417C16.6673 7.36667 13.6757 4.375 10.0007 4.375Z"
                                        fill="#717171" />
                                    <path
                                        d="M10 11.4583C9.65833 11.4583 9.375 11.175 9.375 10.8333V6.66667C9.375 6.325 9.65833 6.04167 10 6.04167C10.3417 6.04167 10.625 6.325 10.625 6.66667V10.8333C10.625 11.175 10.3417 11.4583 10 11.4583Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.5 2.29167H7.5C7.15833 2.29167 6.875 2.00833 6.875 1.66667C6.875 1.325 7.15833 1.04167 7.5 1.04167H12.5C12.8417 1.04167 13.125 1.325 13.125 1.66667C13.125 2.00833 12.8417 2.29167 12.5 2.29167Z"
                                        fill="#717171" />
                                </svg>
                                3 min read
                            </div>
                        </div>

                        <div className="blog-heading">
                            Meta Platforms plans to release free...
                        </div>

                        <div className="para">
                            The parent company of Facebook, Meta Platforms, is introducing software to help deve...
                        </div>
                    </div>
                </div>

                <div className="right-blog">
                    <div onClick={handleViewAllBtn} className="top-blog">
                        <img className="right-blog-img1" src="./right-blog-img1.png" />
                        <div className="top-blog-content">
                            <div className="blog-heading">8 Things You Probably Didn’t Know About Headphones</div>
                            <div className="para">Owning a headphone could mean a different thing for different people.
                                For
                                some, it act as a fashion statement. It’s easy to spot these people, the headphone
                                are
                                almost always just hanging around the neck.</div>
                            <div className="date">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="currentcolor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M6.66602 4.79167C6.32435 4.79167 6.04102 4.50833 6.04102 4.16667V1.66667C6.04102 1.325 6.32435 1.04167 6.66602 1.04167C7.00768 1.04167 7.29102 1.325 7.29102 1.66667V4.16667C7.29102 4.50833 7.00768 4.79167 6.66602 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.334 4.79167C12.9923 4.79167 12.709 4.50833 12.709 4.16667V1.66667C12.709 1.325 12.9923 1.04167 13.334 1.04167C13.6757 1.04167 13.959 1.325 13.959 1.66667V4.16667C13.959 4.50833 13.6757 4.79167 13.334 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 12.0833C6.975 12.0833 6.86667 12.0583 6.76667 12.0167C6.65833 11.975 6.575 11.9167 6.49167 11.8417C6.34167 11.6833 6.25 11.475 6.25 11.25C6.25 11.1417 6.275 11.0333 6.31667 10.9333C6.35833 10.8333 6.41667 10.7417 6.49167 10.6583C6.575 10.5833 6.65833 10.525 6.76667 10.4833C7.06667 10.3583 7.44167 10.425 7.675 10.6583C7.825 10.8167 7.91667 11.0333 7.91667 11.25C7.91667 11.3 7.90833 11.3583 7.9 11.4167C7.89167 11.4667 7.875 11.5167 7.85 11.5667C7.83333 11.6167 7.80833 11.6667 7.775 11.7167C7.75 11.7583 7.70833 11.8 7.675 11.8417C7.51667 11.9917 7.3 12.0833 7.08333 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 12.0833C9.89102 12.0833 9.78268 12.0583 9.68268 12.0167C9.57435 11.975 9.49102 11.9167 9.40768 11.8417C9.25768 11.6833 9.16602 11.475 9.16602 11.25C9.16602 11.1417 9.19102 11.0333 9.23268 10.9333C9.27435 10.8333 9.33268 10.7417 9.40768 10.6583C9.49102 10.5833 9.57435 10.525 9.68268 10.4833C9.98268 10.35 10.3577 10.425 10.591 10.6583C10.741 10.8167 10.8327 11.0333 10.8327 11.25C10.8327 11.3 10.8243 11.3583 10.816 11.4167C10.8077 11.4667 10.791 11.5167 10.766 11.5667C10.7493 11.6167 10.7243 11.6667 10.691 11.7167C10.666 11.7583 10.6243 11.8 10.591 11.8417C10.4327 11.9917 10.216 12.0833 9.99935 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 12.0833C12.809 12.0833 12.7007 12.0583 12.6007 12.0167C12.4923 11.975 12.409 11.9167 12.3257 11.8417C12.2923 11.8 12.259 11.7583 12.2256 11.7167C12.1923 11.6667 12.1673 11.6167 12.1507 11.5667C12.1257 11.5167 12.109 11.4667 12.1007 11.4167C12.0923 11.3583 12.084 11.3 12.084 11.25C12.084 11.0333 12.1757 10.8167 12.3257 10.6583C12.409 10.5833 12.4923 10.525 12.6007 10.4833C12.909 10.35 13.2757 10.425 13.509 10.6583C13.659 10.8167 13.7507 11.0333 13.7507 11.25C13.7507 11.3 13.7423 11.3583 13.734 11.4167C13.7257 11.4667 13.709 11.5167 13.684 11.5667C13.6673 11.6167 13.6423 11.6667 13.609 11.7167C13.584 11.7583 13.5423 11.8 13.509 11.8417C13.3507 11.9917 13.134 12.0833 12.9173 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 15C6.975 15 6.86667 14.975 6.76667 14.9333C6.66667 14.8917 6.575 14.8333 6.49167 14.7583C6.34167 14.6 6.25 14.3833 6.25 14.1667C6.25 14.0583 6.275 13.95 6.31667 13.85C6.35833 13.7417 6.41667 13.65 6.49167 13.575C6.8 13.2667 7.36667 13.2667 7.675 13.575C7.825 13.7333 7.91667 13.95 7.91667 14.1667C7.91667 14.3833 7.825 14.6 7.675 14.7583C7.51667 14.9083 7.3 15 7.08333 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 15C9.78268 15 9.56602 14.9083 9.40768 14.7583C9.25768 14.6 9.16602 14.3833 9.16602 14.1667C9.16602 14.0583 9.19102 13.95 9.23268 13.85C9.27435 13.7417 9.33268 13.65 9.40768 13.575C9.71602 13.2667 10.2827 13.2667 10.591 13.575C10.666 13.65 10.7243 13.7417 10.766 13.85C10.8077 13.95 10.8327 14.0583 10.8327 14.1667C10.8327 14.3833 10.741 14.6 10.591 14.7583C10.4327 14.9083 10.216 15 9.99935 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 15C12.7007 15 12.484 14.9084 12.3257 14.7584C12.2507 14.6834 12.1923 14.5917 12.1507 14.4834C12.109 14.3834 12.084 14.275 12.084 14.1667C12.084 14.0584 12.109 13.95 12.1507 13.85C12.1923 13.7417 12.2507 13.65 12.3257 13.575C12.5173 13.3834 12.809 13.2917 13.0757 13.35C13.134 13.3584 13.184 13.375 13.234 13.4C13.284 13.4167 13.334 13.4417 13.384 13.475C13.4257 13.5 13.4673 13.5417 13.509 13.575C13.659 13.7334 13.7507 13.95 13.7507 14.1667C13.7507 14.3834 13.659 14.6 13.509 14.7584C13.3507 14.9084 13.134 15 12.9173 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M17.0827 8.19997H2.91602C2.57435 8.19997 2.29102 7.91664 2.29102 7.57497C2.29102 7.2333 2.57435 6.94997 2.91602 6.94997H17.0827C17.4243 6.94997 17.7077 7.2333 17.7077 7.57497C17.7077 7.91664 17.4243 8.19997 17.0827 8.19997Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.3333 18.9583H6.66667C3.625 18.9583 1.875 17.2083 1.875 14.1667V7.08333C1.875 4.04167 3.625 2.29167 6.66667 2.29167H13.3333C16.375 2.29167 18.125 4.04167 18.125 7.08333V14.1667C18.125 17.2083 16.375 18.9583 13.3333 18.9583ZM6.66667 3.54167C4.28333 3.54167 3.125 4.7 3.125 7.08333V14.1667C3.125 16.55 4.28333 17.7083 6.66667 17.7083H13.3333C15.7167 17.7083 16.875 16.55 16.875 14.1667V7.08333C16.875 4.7 15.7167 3.54167 13.3333 3.54167H6.66667Z"
                                        fill="#717171" />
                                </svg>
                                August , 8 , 2023
                            </div>
                        </div>
                    </div>

                    <div onClick={handleViewAllBtn} className="top-blog">
                        <img className="right-blog-img1" src="./right-blog-img2.png" />
                        <div className="top-blog-content">
                            <div className="blog-heading">8 Things You Probably Didn’t Know About Headphones</div>
                            <div className="para">Owning a headphone could mean a different thing for different people.
                                For
                                some, it act as a fashion statement. It’s easy to spot these people, the headphone
                                are
                                almost always just hanging around the neck.</div>
                            <div className="date">
                                <svg width="20" height="20" viewBox="0 0 20 20" fill="currentcolor"
                                    xmlns="http://www.w3.org/2000/svg">
                                    <path
                                        d="M6.66602 4.79167C6.32435 4.79167 6.04102 4.50833 6.04102 4.16667V1.66667C6.04102 1.325 6.32435 1.04167 6.66602 1.04167C7.00768 1.04167 7.29102 1.325 7.29102 1.66667V4.16667C7.29102 4.50833 7.00768 4.79167 6.66602 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.334 4.79167C12.9923 4.79167 12.709 4.50833 12.709 4.16667V1.66667C12.709 1.325 12.9923 1.04167 13.334 1.04167C13.6757 1.04167 13.959 1.325 13.959 1.66667V4.16667C13.959 4.50833 13.6757 4.79167 13.334 4.79167Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 12.0833C6.975 12.0833 6.86667 12.0583 6.76667 12.0167C6.65833 11.975 6.575 11.9167 6.49167 11.8417C6.34167 11.6833 6.25 11.475 6.25 11.25C6.25 11.1417 6.275 11.0333 6.31667 10.9333C6.35833 10.8333 6.41667 10.7417 6.49167 10.6583C6.575 10.5833 6.65833 10.525 6.76667 10.4833C7.06667 10.3583 7.44167 10.425 7.675 10.6583C7.825 10.8167 7.91667 11.0333 7.91667 11.25C7.91667 11.3 7.90833 11.3583 7.9 11.4167C7.89167 11.4667 7.875 11.5167 7.85 11.5667C7.83333 11.6167 7.80833 11.6667 7.775 11.7167C7.75 11.7583 7.70833 11.8 7.675 11.8417C7.51667 11.9917 7.3 12.0833 7.08333 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 12.0833C9.89102 12.0833 9.78268 12.0583 9.68268 12.0167C9.57435 11.975 9.49102 11.9167 9.40768 11.8417C9.25768 11.6833 9.16602 11.475 9.16602 11.25C9.16602 11.1417 9.19102 11.0333 9.23268 10.9333C9.27435 10.8333 9.33268 10.7417 9.40768 10.6583C9.49102 10.5833 9.57435 10.525 9.68268 10.4833C9.98268 10.35 10.3577 10.425 10.591 10.6583C10.741 10.8167 10.8327 11.0333 10.8327 11.25C10.8327 11.3 10.8243 11.3583 10.816 11.4167C10.8077 11.4667 10.791 11.5167 10.766 11.5667C10.7493 11.6167 10.7243 11.6667 10.691 11.7167C10.666 11.7583 10.6243 11.8 10.591 11.8417C10.4327 11.9917 10.216 12.0833 9.99935 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 12.0833C12.809 12.0833 12.7007 12.0583 12.6007 12.0167C12.4923 11.975 12.409 11.9167 12.3257 11.8417C12.2923 11.8 12.259 11.7583 12.2256 11.7167C12.1923 11.6667 12.1673 11.6167 12.1507 11.5667C12.1257 11.5167 12.109 11.4667 12.1007 11.4167C12.0923 11.3583 12.084 11.3 12.084 11.25C12.084 11.0333 12.1757 10.8167 12.3257 10.6583C12.409 10.5833 12.4923 10.525 12.6007 10.4833C12.909 10.35 13.2757 10.425 13.509 10.6583C13.659 10.8167 13.7507 11.0333 13.7507 11.25C13.7507 11.3 13.7423 11.3583 13.734 11.4167C13.7257 11.4667 13.709 11.5167 13.684 11.5667C13.6673 11.6167 13.6423 11.6667 13.609 11.7167C13.584 11.7583 13.5423 11.8 13.509 11.8417C13.3507 11.9917 13.134 12.0833 12.9173 12.0833Z"
                                        fill="#717171" />
                                    <path
                                        d="M7.08333 15C6.975 15 6.86667 14.975 6.76667 14.9333C6.66667 14.8917 6.575 14.8333 6.49167 14.7583C6.34167 14.6 6.25 14.3833 6.25 14.1667C6.25 14.0583 6.275 13.95 6.31667 13.85C6.35833 13.7417 6.41667 13.65 6.49167 13.575C6.8 13.2667 7.36667 13.2667 7.675 13.575C7.825 13.7333 7.91667 13.95 7.91667 14.1667C7.91667 14.3833 7.825 14.6 7.675 14.7583C7.51667 14.9083 7.3 15 7.08333 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M9.99935 15C9.78268 15 9.56602 14.9083 9.40768 14.7583C9.25768 14.6 9.16602 14.3833 9.16602 14.1667C9.16602 14.0583 9.19102 13.95 9.23268 13.85C9.27435 13.7417 9.33268 13.65 9.40768 13.575C9.71602 13.2667 10.2827 13.2667 10.591 13.575C10.666 13.65 10.7243 13.7417 10.766 13.85C10.8077 13.95 10.8327 14.0583 10.8327 14.1667C10.8327 14.3833 10.741 14.6 10.591 14.7583C10.4327 14.9083 10.216 15 9.99935 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M12.9173 15C12.7007 15 12.484 14.9084 12.3257 14.7584C12.2507 14.6834 12.1923 14.5917 12.1507 14.4834C12.109 14.3834 12.084 14.275 12.084 14.1667C12.084 14.0584 12.109 13.95 12.1507 13.85C12.1923 13.7417 12.2507 13.65 12.3257 13.575C12.5173 13.3834 12.809 13.2917 13.0757 13.35C13.134 13.3584 13.184 13.375 13.234 13.4C13.284 13.4167 13.334 13.4417 13.384 13.475C13.4257 13.5 13.4673 13.5417 13.509 13.575C13.659 13.7334 13.7507 13.95 13.7507 14.1667C13.7507 14.3834 13.659 14.6 13.509 14.7584C13.3507 14.9084 13.134 15 12.9173 15Z"
                                        fill="#717171" />
                                    <path
                                        d="M17.0827 8.19997H2.91602C2.57435 8.19997 2.29102 7.91664 2.29102 7.57497C2.29102 7.2333 2.57435 6.94997 2.91602 6.94997H17.0827C17.4243 6.94997 17.7077 7.2333 17.7077 7.57497C17.7077 7.91664 17.4243 8.19997 17.0827 8.19997Z"
                                        fill="#717171" />
                                    <path
                                        d="M13.3333 18.9583H6.66667C3.625 18.9583 1.875 17.2083 1.875 14.1667V7.08333C1.875 4.04167 3.625 2.29167 6.66667 2.29167H13.3333C16.375 2.29167 18.125 4.04167 18.125 7.08333V14.1667C18.125 17.2083 16.375 18.9583 13.3333 18.9583ZM6.66667 3.54167C4.28333 3.54167 3.125 4.7 3.125 7.08333V14.1667C3.125 16.55 4.28333 17.7083 6.66667 17.7083H13.3333C15.7167 17.7083 16.875 16.55 16.875 14.1667V7.08333C16.875 4.7 15.7167 3.54167 13.3333 3.54167H6.66667Z"
                                        fill="#717171" />
                                </svg>
                                August , 8 , 2023
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}

export default Blogs;